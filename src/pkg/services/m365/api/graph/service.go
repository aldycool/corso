...
func CreateAdapter(
	tenant, client, secret, oboRefreshToken, oboServiceId, oboServiceSecret string,
	counter *count.Bus,
	opts ...Option,
) (abstractions.RequestAdapter, error) {
	auth, err := GetAuth(tenant, client, secret, oboRefreshToken, oboServiceId, oboServiceSecret)
	if err != nil {
		return nil, err
	}
